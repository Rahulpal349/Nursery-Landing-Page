...
const ProductRelated = ({ currentPlantId }) => {
  ...
  useEffect(() => {
    ...
    // Filter out the current plant and get up to 10 other plants
    const filtered = allPlants.filter(p => p.id !== currentPlantId).slice(0, 10);
  ...
  const scrollLeft = () => {
    if (carouselRef.current) {
      const cardWidth = carouselRef.current.firstElementChild.offsetWidth + 20;
    ...
    <section className="product-related-section">
      <div className="container">
        ...
        <div className="related-carousel-container">
          ...
          <button className="carousel-nav-btn prev" onClick={scrollLeft}>
          ...
          <button className="carousel-nav-btn next" onClick={scrollRight}>
